...
# ╔═╡ c4cf77d9-a63e-409e-b962-c508534c574a
using Images

# ╔═╡ 62fae257-260e-482c-8874-42e5877df5e4
using Plots

# ╔═╡ bfad1a01-1b3a-4b8a-bb49-dee1d67b6a46
using FileIO

# ╔═╡ a1d96c6e-2afd-11ee-3dac-3df8d7bcbf43
md"""
# Generate previews for polylux examples
"""

# ╔═╡ a144f519-0cba-456b-98ae-e35b4b85845c
clamp(2, 4, 3)

# ╔═╡ ed3ed8f7-149e-4295-8255-479ca0922c0d
min(max(2, 4), 3)

# ╔═╡ d6334a4c-6d55-4eaf-852a-7b6489c44305
function montage(imgs, annotation)
	n = length(imgs)

	annotations = if annotation == :idx
		string.(1:n)
	elseif annotation == :none
		fill("", n)
	elseif annotation isa String
		annotation .* string.(1:n)
	else
		annotation
	end
	...
	plt = plot(
		...
		xticks = false,
		yticks = false,
		top_margin = all(iszero ∘ length, annotations) ? (0, :pt) : (15a, :pt),
		left_margin = (0, :pt),
		right_margin = (0, :pt),
		bottom_margin = (0, :pt),
		layout = (b, a),
		...
		annotationfontsize = 15a
	)
	for (img, idx, annot) in zip(imgs, idcs, annotations)
		plot!(plt[reverse(Tuple(idx))...], img, annotation = (0, 0, string(annot)))
	end
	plt
end

# ╔═╡ e0a254fd-c894-462a-82a3-94b39bb5bd04
function generate_previews(items)
	ancestors = (pwd() |> splitpath) .* "/" |> cumprod |> reverse .|> p -> p[2:end]
	root_idx = findfirst(p -> isfile(joinpath(p, "polylux.typ")), ancestors)
	@assert !isnothing(root_idx)
	root = ancestors[root_idx]
	@info "Found root" root
	cd(root) do
		for item in items
			mktempdir() do tmp
				...
				@info "Generated preview" item.input
			end
		end
	end
end

# ╔═╡ 699acd2b-31b3-4b90-830c-46222ebf9f27
typ2png(;path, file, label = :none) = (;
	input = joinpath(path, file * ".typ"),
	output = joinpath(path, file * ".png"),
	label
)

# ╔═╡ 95a869c8-8596-4834-a9a1-74d29da2432c
typ2png(path = "book/src/dynamic", file = "rules-array", label = "subslide ")
...
generate_previews([
	...
	typ2png(path = diy, file = "quiz"),
	typ2png(path = dynamic, file = "only-uncover", label = "subslide "),
	typ2png(path = dynamic, file = "rule-array", label = "subslide "),
	typ2png(path = dynamic, file = "rule-interval", label = "subslide "),
	typ2png(path = dynamic, file = "rule-string", label = "subslide "),
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "bbdbaf8b78ea8bb03c170917adaac6ac4fd540ad"
...
git-tree-sha1 = "0eab7832affd4fcf1329c021ed9341765254704e"
